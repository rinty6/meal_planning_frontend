...
const GuidanceContent = ({ onClose }: GuidanceContentProps) => (
  <View className="flex-1 bg-appBackground">
    <View className="flex-row items-center px-5 py-4 border-b border-borderSoft bg-surface">
      {onClose ? (
        <TouchableOpacity
          onPress={onClose}
          className="w-10 h-10 rounded-full bg-neutralSoft items-center justify-center mr-3"
        >
          <Ionicons name="close" size={22} color="#0B2149" />
        </TouchableOpacity>
      ) : null}
