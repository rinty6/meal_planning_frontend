// Reusable notification inbox for both Home and Profile.
// Created so the Home bell can show messages without pushing the Profile tab
// into a nested notifications route, which previously made Profile feel stuck.
...
const NotificationMessage = ({
  ...
  const loadNotifications = useCallback(
    async ({
      ...
      try {
        const response = await authedFetch(`/api/notifications/${userId}`, {
  ...
  return (
    <View className="flex-1 bg-[#F3F4F6]">
      <View className="px-5 py-4 border-b border-gray-200 flex-row items-center justify-between">
        <Text className="text-3xl font-bold text-[#111827]">{title}</Text>
        {onClose ? (
          <TouchableOpacity onPress={onClose} className="p-2">
            <Ionicons name={closeIcon} size={22} color="#6B7280" />
          </TouchableOpacity>
        ) : null}
      </View>
          ...
          <FlatList
            ...
            refreshControl={
              <RefreshControl
                ...
                onRefresh={() =>
                  loadNotifications({ isRefresh: true, showSpinner: false, force: true })
                }
                tintColor="#2563EB"
              />
            }
